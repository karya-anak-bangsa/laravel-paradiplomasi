@if ($misiPermanenAsean->latitude && $misiPermanenAsean->longitude)
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const lokasi = [{{ $misiPermanenAsean->latitude }}, {{ $misiPermanenAsean->longitude }}];
            ...
            L.marker(lokasi).addTo(peta)
                .bindPopup(
                    '<div class="fw-bold mb-1">Misi Permanen Negara ASEAN {{ $misiPermanenAsean->nama_negara }}</div>' +
                    '<a href="https://www.google.com/maps?q={{ $misiPermanenAsean->latitude }},{{ $misiPermanenAsean->longitude }}" target="_blank" rel="noopener" class="text-primary">' +
